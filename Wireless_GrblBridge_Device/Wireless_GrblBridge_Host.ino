...
String mqtt_username="CNCDEVICE";
String mqtt_password="20250701";


String mqtt_remote_devicename="device/PCHost/data";
...
void callback(char* topic, byte* payload, unsigned int length) {
    ...
    for (int i = 0; i < length; i++) {
        ...
        grbl+=(char)payload[i];
        if(payload[i]=='k'||payload[i]==']'||payload[i]==')'||(grbl.length()>128)||payload[i]=='>'){
         Serial.println(grbl);
         grbl="";
        }
